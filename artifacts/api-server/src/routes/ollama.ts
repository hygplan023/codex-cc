...
import { docker, ollamaApiUrl, toServerReachableOllamaUrl } from "../lib/docker";
import os from "os";
...
router.get("/ollama/status", async (req, res) => {
  try {
    ...
    if (running) {
      try {
        const resp = await fetch(`${ollamaApiUrl(port)}/api/tags`, {
...
router.get("/ollama/models", async (req, res) => {
  try {
    ...
    const port = portBinding?.PublicPort ?? DEFAULT_PORT;
    const resp = await fetch(`${ollamaApiUrl(port)}/api/tags`, { signal: AbortSignal.timeout(5000) });
...
router.post("/ollama/models/pull", async (req, res) => {
  try {
    ...
    try {
      const resp = await fetch(`${ollamaApiUrl(port)}/api/pull`, {
        method: "POST",
...
router.delete("/ollama/models/:name", async (req, res) => {
  try {
    ...
    const resp = await fetch(`${ollamaApiUrl(port)}/api/delete`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
...
router.post("/ollama/test-connection", async (req, res) => {
  try {
    ...
    // 浏览器传入的通常是 localhost；后端在容器内需改写为可达主机名
    const target = toServerReachableOllamaUrl(url);
    const startTime = Date.now();
    let resp: Response;
    try {
      resp = await fetch(`${target}/api/tags`, { signal: AbortSignal.timeout(6000) });
...
router.post("/ollama/launch-check", async (req, res) => {
  try {
    ...
    if (containerRunning) {
      try {
        const resp = await fetch(`${ollamaApiUrl(port)}/api/tags`, { signal: AbortSignal.timeout(4000) });
        if (resp.ok) {
          apiReachable = true;
          const data = (await resp.json()) as { models?: Array<{ name: string }> };
    ...
    const codexLaunch = "codex --dangerously-bypass-approvals-and-sandbox";
    const commands = {
      codex: {
        configToml: `# ~/.codex/config.toml — 本地 Ollama 配置\nmodel = "${actualModel}"\nmodel_provider = "ollama"\n\n[model_providers.ollama]\nname = "Ollama (本地)"\nbase_url = "${openaiBaseUrl}"\nwire_api = "chat"`,
        win: codexLaunch,
        mac: codexLaunch,
        linux: codexLaunch,
      },
