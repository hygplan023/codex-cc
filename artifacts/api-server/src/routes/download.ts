...
import { toServerReachableOllamaUrl } from "../lib/docker";
...
router.post("/codex/check", async (req, res) => {
  ...
  // 后端在容器内时把 localhost 改写为 host.docker.internal 等可达主机名
  const probeUrl = toServerReachableOllamaUrl(ollamaUrl);
  ...
  try {
    const resp = await fetch(`${probeUrl}/api/tags`, { signal: AbortSignal.timeout(3000) });
    if (resp.ok) {
  ...
  const openaiBaseUrl = cleanUrl.endsWith("/v1") ? cleanUrl : `${cleanUrl}/v1`;
  const firstModel = ollamaModels[0] || "qwen2.5-coder:7b";

  // 现代 Codex CLI 使用 ~/.codex/config.toml；直连 Ollama 必须 wire_api = "chat"。
  const configToml = `# ~/.codex/config.toml — 本地 Ollama 配置
model = "${firstModel}"
...
wire_api = "chat"`;
  ...
  return res.json({
    ...
    openaiBaseUrl,
    readyToRun: ollamaOk,
    configToml: ollamaOk && ollamaModels.length > 0 ? configToml : null,
