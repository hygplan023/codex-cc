          ...
          <CardContent className="pt-4 space-y-3">
            ...
            {checkResult && (
              <div className="space-y-2">
                ...
                {checkResult.ollamaOk && checkResult.configToml && (
                  <div className="space-y-1.5 pt-1">
                    <p className="text-xs text-muted-foreground">把以下内容保存为 <code className="text-cyan-300">~/.codex/config.toml</code>：</p>
                    <div className="relative group">
                      <pre className="bg-[#0d1117] border border-[#30363d] rounded p-2.5 text-xs font-mono text-gray-300 overflow-x-auto leading-relaxed">
                        {checkResult.configToml}
                      </pre>
                      <Button size="sm" variant="ghost" onClick={() => copy(checkResult.configToml!)}
                        className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 h-6 text-xs text-muted-foreground px-2">
                        <Copy className="w-3 h-3 mr-1" />复制
                      </Button>
                    </div>
                    <p className="text-xs text-muted-foreground">然后在终端运行：</p>
                    <div className="relative group">
                      <pre className="bg-[#0d1117] border border-[#30363d] rounded p-2.5 text-xs font-mono text-cyan-300 overflow-x-auto leading-relaxed">
                        {checkResult.launchCommand}
                      </pre>
                      <Button size="sm" variant="ghost" onClick={() => copy(checkResult.launchCommand)}
                        className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 h-6 text-xs text-muted-foreground px-2">
                        <Copy className="w-3 h-3 mr-1" />复制
                      </Button>
                    </div>
                    <p className="text-xs text-muted-foreground">💡 想自动写入配置 + MCP + skill？到「Ollama 管理」页点「一键启动 Codex」。</p>
                  </div>
                )}
              </div>
            )}
...
pnpm --filter @workspace/docker-manager run dev`} />
          ...
          {activeTab === 2 && (
            <div className="space-y-6">
              ...
              <div className="bg-green-500/5 border border-green-500/20 rounded-lg p-4 text-sm text-green-300 flex items-start gap-2">
                <Zap className="w-4 h-4 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="font-semibold">最省事：用「一键配置」</p>
                  <p className="text-xs text-green-400/90 mt-1">到「Ollama 管理」页点「一键启动 Codex」，管理中心会自动帮你写好 config.toml、MCP 工具和 AGENTS.md skill。下面是手动配置说明，供进阶用户参考。</p>
                </div>
              </div>

              <Step num={2} title="配置使用 Ollama 本地模型（config.toml）">
                <p className="text-muted-foreground">新版 Codex CLI 使用 <code className="font-mono text-cyan-400">~/.codex/config.toml</code>（TOML 格式）。2026.02 起 Codex <span className="text-amber-300">只支持 Responses API</span>，必须设 <code className="font-mono text-cyan-400">wire_api = "responses"</code>（旧的 <code className="font-mono">chat</code> 已被移除）。Ollama 需 <span className="text-amber-300">≥ 0.13.3</span> 才提供 <code className="font-mono text-cyan-400">/v1/responses</code> 接口。</p>
                <CopyBlock code={`# ~/.codex/config.toml
# 使用 Ollama 本地模型（无需 OpenAI API Key）

model = "qwen2.5-coder:7b"   # 替换为你拉取的模型名
model_provider = "ollama-local"   # Codex 内置 ollama 不可覆盖，自定义须改名

# Codex 内置了名为 ollama 的 provider 且禁止覆盖，自定义 provider 必须改名
[model_providers.ollama-local]
name = "Ollama (本地)"
base_url = "http://localhost:11434/v1"
wire_api = "responses"       # 关键：新版 Codex 只支持 responses（需 Ollama ≥ 0.13.3）`} />
                <p className="text-muted-foreground">配置好后，在终端运行（Windows cmd 同样适用）：</p>
                <CopyBlock code={`codex --dangerously-bypass-approvals-and-sandbox`} />
              </Step>

              <Step num={3} title="启用 MCP 插件（解锁 Codex 扩展能力）">
                <p className="text-muted-foreground">MCP 是 Model Context Protocol，让 Codex 能调用外部工具。新版用 <code className="font-mono text-cyan-400">[mcp_servers.*]</code>（下划线）配置。以下为确实存在的官方包：</p>
                <div className="space-y-3">
                  {[
                    { name: "文件系统访问", npm: "@modelcontextprotocol/server-filesystem", desc: "让 Codex 读写本地文件" },
                    { name: "长期记忆", npm: "@modelcontextprotocol/server-memory", desc: "知识图谱式记忆，跨会话保留上下文" },
                    { name: "逐步思考", npm: "@modelcontextprotocol/server-sequential-thinking", desc: "把复杂问题拆解为分步推理" },
                  ].map((plugin) => (
                    <div key={plugin.name} className="border border-border rounded p-3 bg-background/40 space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <Button size="sm" variant="ghost" className="h-6 text-xs text-muted-foreground px-2"
                          onClick={() => copy(plugin.npm)}>
                          <Copy className="w-3 h-3 mr-1" /> 复制包名
                        </Button>
                      </div>
                      <p className="text-xs text-muted-foreground">{plugin.desc}</p>
                      <code className="text-xs font-mono text-cyan-300">{plugin.npm}</code>
                    </div>
                  ))}
                </div>

                <p className="text-muted-foreground mt-2">在 <code className="font-mono text-cyan-400">~/.codex/config.toml</code> 末尾追加（Windows 用 <code className="font-mono">cmd /c</code> 包一层更稳）：</p>
                <CopyBlock label="Windows：" code={`[mcp_servers.filesystem]
command = "cmd"
...
args = ["/c", "npx", "-y", "@modelcontextprotocol/server-memory"]`} />
                <CopyBlock label="macOS / Linux：" code={`[mcp_servers.filesystem]
command = "npx"
...
args = ["-y", "@modelcontextprotocol/server-memory"]`} />
              ...
              <Step num={4} title="写入编程助手 skill（AGENTS.md）">
                <p className="text-muted-foreground">把全局指令写入 <code className="font-mono text-cyan-400">~/.codex/AGENTS.md</code>，对所有项目生效（约定、工作流、安全边界）。「一键配置」会自动写入；也可手动创建：</p>
                <CopyBlock code={`# 全局编程助手指令（AGENTS.md）

你是一名严谨、务实的资深工程师助手，全程使用简体中文沟通。
优先交付可运行的代码，遵循项目既有结构与风格；改动小而聚焦。
涉及删除文件、批量重构、改依赖等高风险操作前先说明影响并征得同意。
不打印任何密钥/令牌；不执行未确认的破坏性命令。`} />
              </Step>

              <Step num={5} title="测试 Codex 是否正常工作">
                <CopyBlock code={`# 启动（绕过 Windows 沙盒报错）
codex --dangerously-bypass-approvals-and-sandbox

# 然后输入：用 Python 写一个计算斐波那契数列的函数
          ...
          {activeTab === 3 && (
            <div className="space-y-4">
              {[
                ...
                {
                  q: "Codex 启动报沙盒 / 权限 / approvals 相关错误（尤其在 Windows）",
                  a: "用 codex --dangerously-bypass-approvals-and-sandbox 启动。Windows 上 Codex 的沙盒机制经常报错，这个参数会跳过审批与沙盒，让 Codex 直接运行命令和读写文件。仅在你信任本机环境时使用。",
                },
                {
                  q: "连上了但模型一直报错 / 无响应（直连 Ollama）",
                  a: "新版 Codex（2026.02 起）只支持 Responses API：config.toml 里必须设 wire_api = \"responses\"（旧的 \"chat\" 已被移除，仍用 chat 会启动报错）。同时 Ollama 需升级到 ≥ 0.13.3，否则 /v1/responses 返回 404。用「一键配置」会自动写对。",
                },
                {
                  q: "报错 does not support tools（模型不支持工具）",
                  a: "Codex 默认会调用函数/工具，所以必须选用支持 tools 的模型。推荐 qwen2.5-coder、llama3.1、qwen3 等；像 codestral 这类只支持补全（completion/insert）的模型会报「does not support tools」。换一个支持工具调用的模型即可。",
                },
                {
                  q: "报错 reserved built-in provider IDs: ollama",
                  a: "新版 Codex 内置了名为 ollama 的 provider 且禁止覆盖。自定义 provider 必须改用别的名字（本中心用 ollama-local），并让 model_provider 指向同一个名字。用「一键配置」会自动写对。",
                },
                {
                  q: "模型拉取卡在「0%」很长时间",
