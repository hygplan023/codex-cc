...
interface LaunchResult {
  ...
  commands: {
    codex: { configToml: string; win: string; mac: string; linux: string };
...
interface CodexEnv {
  ...
  uvInstalled: boolean;
  uvVersion: string;
  mcpCatalog: { id: string; name: string; desc: string; note: string | null; defaultOn: boolean }[];
}
...
function LaunchModal({ open, onClose, tool, selectedModel }: {
  ...
  const [mcpSel, setMcpSel] = useState<Record<string, boolean>>({});
  const [fsDir, setFsDir] = useState(".");
  const [customMcp, setCustomMcp] = useState("");
  const [includeSkills, setIncludeSkills] = useState(true);
  const [applying, setApplying] = useState(false);
  const [applyResult, setApplyResult] = useState<CodexApplyResult | null>(null);
  ...
  const checkAndLoad = useCallback(async () => {
    ...
    setApplyResult(null);
    try {
      const lcPromise = fetch("/api/ollama/launch-check", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: selectedModel, tool }),
      }).then((r) => r.json() as Promise<LaunchResult>);
      const envPromise = isCodex
        ? fetch("/api/codex/env").then((r) => r.json() as Promise<CodexEnv>)
        : Promise.resolve(null);
      const [lc, envData] = await Promise.all([lcPromise, envPromise]);
      setResult(lc);
      if (envData) {
        setEnv(envData);
        setMcpSel((prev) =>
          Object.keys(prev).length
            ? prev
            : Object.fromEntries(envData.mcpCatalog.map((m) => [m.id, m.defaultOn])));
      }
    } catch {
  ...
  }, [selectedModel, tool, toast, isCodex]);
  ...
  const handleApply = async () => {
    ...
    setApplyResult(null);
    try {
      const mcpServers = Object.entries(mcpSel).filter(([, v]) => v).map(([k]) => k);
      const resp = await fetch("/api/codex/apply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: result.resolvedModel,
          ollamaUrl: result.baseUrl,
          mcpServers,
          includeSkills,
          filesystemDir: fsDir,
          customMcpToml: customMcp,
        }),
      });
      const data = await resp.json() as CodexApplyResult;
      setApplyResult(data);
      if (data.ok) toast({ title: "✅ 已写入 ~/.codex 配置" });
      else if (data.notLocal) toast({ variant: "destructive", title: "管理中心运行在容器内", description: "已生成可在本机执行的脚本/配置" });
      else toast({ variant: "destructive", title: "写入失败", description: data.message });
    } catch {
      toast({ variant: "destructive", title: "写入失败", description: "无法连接 API 服务器" });
    } finally {
      setApplying(false);
    }
  };
      ...
      <DialogContent className="bg-[#0d1117] border border-[#30363d] max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className={cn("flex items-center gap-2", isCodex ? "text-cyan-400" : "text-purple-400")}>
            ...
            {isCodex ? "一键配置 Codex CLI — 本地 Ollama 模式" : "一键启动 Claude Code — 本地模型模式"}
          </DialogTitle>
          <DialogDescription className="text-muted-foreground text-xs">
            {isCodex
              ? "自动写入 ~/.codex/config.toml（指向本地 Ollama）、MCP 工具与 AGENTS.md skill，配置完即可在终端运行 codex"
              : "配置 Claude Code 接入 Ollama 本地模型，替代 Anthropic 云端 API"}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-2">
          ...
          <div className="space-y-2">
            ...
            {result && (
              <div className="space-y-1.5">
                ...
                {/* Codex env line */}
                {isCodex && env && (
                  <div className={cn("flex items-start gap-2.5 text-xs rounded-lg px-3 py-2.5 border",
                    env.isLocal ? "bg-green-500/10 border-green-500/20" : "bg-amber-500/10 border-amber-500/20")}>
                    {env.isLocal ? <CheckCircle2 className="w-3.5 h-3.5 text-green-400 flex-shrink-0 mt-0.5" /> : <AlertTriangle className="w-3.5 h-3.5 text-amber-400 flex-shrink-0 mt-0.5" />}
                    <div>
                      <span className={cn("font-medium", env.isLocal ? "text-green-300" : "text-amber-300")}>运行环境</span>
                      <span className="text-muted-foreground ml-1">—</span>
                      <span className={cn("ml-1", env.isLocal ? "text-green-400" : "text-amber-400")}>
                        {env.isLocal ? `本机原生运行，可写入 ${env.configPath}` : "运行在 Docker 容器内，无法自动写宿主机配置"}
                      </span>
                      {env.isLocal && env.configExists && (
                        <p className="text-amber-400/80 mt-0.5 flex items-center gap-1">
                          <ChevronRight className="w-3 h-3" />已存在配置，写入前会自动备份
                        </p>
                      )}
                    </div>
                  </div>
          ...
          {result && !result.ready && (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3 text-xs text-amber-300 flex items-start gap-2">
              ...
              <div>
                <p className="font-medium mb-1">环境未就绪</p>
                <p>请解决上述红色项目后重新检测。确保 Ollama 已部署、端口可达、已拉取至少一个模型。</p>
              </div>
            </div>
          )}

          {/* ── Claude 分支：保持原启动命令复制流程 ───────────────────────── */}
          {!isCodex && result && result.ready && (
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="mt-2 relative group">
                  <pre className="bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-xs font-mono text-purple-300 leading-relaxed overflow-x-auto whitespace-pre-wrap">
                    {getClaudeCommand()}
                  </pre>
                  <Button size="sm" variant="ghost" onClick={() => copy(getClaudeCommand(), `cmd-${platformTab}`)}
          ...
          {isCodex && result && result.ready && env && (
            <div className="space-y-4">
              ...
              {env.codexInstalled && (
                <div className="flex items-center gap-2 text-xs text-green-400 bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-2">
                  <CheckCircle2 className="w-3.5 h-3.5 flex-shrink-0" /> 已安装 Codex CLI：<span className="font-mono">{env.codexVersion}</span>
                </div>
              )}

              {/* MCP 工具选择 */}
              <div>
                <p className="text-xs font-semibold text-white mb-1">① 选择要启用的 MCP 工具</p>
                <p className="text-xs text-muted-foreground mb-2">勾选后会写入 config.toml 的 [mcp_servers.*]，让 Codex 具备读写文件、记忆等能力</p>
                <div className="space-y-2">
                  {env.mcpCatalog.map((m) => {
                    const on = !!mcpSel[m.id];
                    return (
                      <div key={m.id}>
                        <button type="button"
                          onClick={() => setMcpSel((p) => ({ ...p, [m.id]: !on }))}
                          className={cn("w-full text-left flex items-start gap-2.5 rounded-lg px-3 py-2.5 border transition-colors",
                            on ? "bg-cyan-500/10 border-cyan-500/30" : "bg-background/30 border-border hover:border-cyan-500/20")}>
                          <span className={cn("mt-0.5 w-4 h-4 rounded border flex items-center justify-center flex-shrink-0",
                            on ? "bg-cyan-500 border-cyan-500" : "border-muted-foreground/50")}>
                            {on && <CheckCircle2 className="w-3 h-3 text-white" />}
                          </span>
                          <div className="min-w-0">
                            <span className="text-sm text-white font-medium">{m.name}</span>
                            <p className="text-xs text-muted-foreground">{m.desc}</p>
                            {m.note && <p className="text-xs text-amber-400/80 mt-0.5">⚠ {m.note}</p>}
                          </div>
                        </button>
                        ...
                        {m.id === "filesystem" && on && (
                          <div className="mt-1.5 ml-7 space-y-1">
                            <Label htmlFor="fs-dir" className="text-xs text-muted-foreground">工作目录（Codex 可读写的根目录）</Label>
                            <Input id="fs-dir" value={fsDir} onChange={(e) => setFsDir(e.target.value)}
                              placeholder="." spellCheck={false}
                              className="h-8 text-xs font-mono bg-[#0d1117] border-[#30363d]" />
                            <p className="text-xs text-muted-foreground">
                              默认 <code className="text-cyan-300">.</code>（启动 codex 时所在目录）。也可填绝对路径，如
                              <code className="text-cyan-300"> C:\\Projects\\my-app</code> 或 <code className="text-cyan-300"> /home/me/code</code>
                            </p>
                          </div>
                        )}
                        ...
                        {m.id === "git" && on && (
                          env.uvInstalled ? (
                            <div className="mt-1.5 ml-7 flex items-center gap-1.5 text-xs text-green-400">
                              <CheckCircle2 className="w-3 h-3 flex-shrink-0" /> 已检测到 uv：<span className="font-mono">{env.uvVersion}</span>
                            </div>
                          ) : (
                            <div className="mt-1.5 ml-7 space-y-1.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
                              <p className="flex items-center gap-1.5 font-medium"><AlertTriangle className="w-3.5 h-3.5 flex-shrink-0" />未检测到 uv，git MCP 需要先安装</p>
                              <p className="text-amber-300/80">在本机终端运行以下命令安装 uv（Python 工具链）：</p>
                              <div className="relative group">
                                <pre className="bg-[#0d1117] rounded p-2 font-mono text-gray-300 overflow-x-auto">{env.platform === "win32" ? "powershell -c \"irm https://astral.sh/uv/install.ps1 | iex\"" : "curl -LsSf https://astral.sh/uv/install.sh | sh"}</pre>
                                <Button size="sm" variant="ghost"
                                  onClick={() => copy(env.platform === "win32" ? "powershell -c \"irm https://astral.sh/uv/install.ps1 | iex\"" : "curl -LsSf https://astral.sh/uv/install.sh | sh", "install-uv")}
                                  className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 h-6 text-xs px-2 text-muted-foreground">
                                  {copiedKey === "install-uv" ? <CheckCircle2 className="w-3 h-3 text-green-400" /> : <Copy className="w-3 h-3" />}
                                </Button>
                              </div>
                              <p className="text-amber-300/70">安装后重开终端，未装 uv 仍可写入配置，但 git MCP 启动会失败。</p>
                            </div>
                          )
                        )}
                      </div>
              ...
              {/* 自定义 MCP 片段 */}
              <div>
                <p className="text-xs font-semibold text-white mb-1">② 粘贴自定义 MCP 工具（可选）</p>
                <p className="text-xs text-muted-foreground mb-2">
                  把你自己的 <code className="text-cyan-300">[mcp_servers.*]</code> TOML 片段粘到这里，会原样合并进生成的 config.toml
                </p>
                <textarea value={customMcp} onChange={(e) => setCustomMcp(e.target.value)}
                  spellCheck={false} rows={5}
                  placeholder={'[mcp_servers.my_tool]\ncommand = "npx"\nargs = ["-y", "my-mcp-server"]'}
                  className="w-full rounded-lg bg-[#0d1117] border border-[#30363d] p-3 text-xs font-mono text-gray-300 resize-y focus:outline-none focus:border-cyan-500/40" />
              </div>

              {/* Skill 选择 */}
              <button type="button" onClick={() => setIncludeSkills((v) => !v)}
                className={cn("w-full text-left flex items-start gap-2.5 rounded-lg px-3 py-2.5 border transition-colors",
                  includeSkills ? "bg-cyan-500/10 border-cyan-500/30" : "bg-background/30 border-border hover:border-cyan-500/20")}>
                <span className={cn("mt-0.5 w-4 h-4 rounded border flex items-center justify-center flex-shrink-0",
                  includeSkills ? "bg-cyan-500 border-cyan-500" : "border-muted-foreground/50")}>
                  {includeSkills && <CheckCircle2 className="w-3 h-3 text-white" />}
                </span>
                <div>
                  <span className="text-sm text-white font-medium">② 写入中文编程助手 skill</span>
                  <p className="text-xs text-muted-foreground">写入 ~/.codex/AGENTS.md（约定、工作流、安全边界），对所有项目生效</p>
                </div>
              </button>
              ...
              <Button onClick={handleApply} disabled={applying}
                ...
                {applying ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />正在写入...</>
                  : env.isLocal ? <><Rocket className="w-4 h-4 mr-2" />一键写入 ~/.codex 配置</>
                    : <><Rocket className="w-4 h-4 mr-2" />生成本机配置脚本</>}
              </Button>

              {/* 写入结果：成功（本机） */}
              {applyResult && applyResult.ok && (
                <div className="space-y-3">
                  <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-xs text-green-300 space-y-1.5">
                    <p className="font-medium flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" />配置已写入</p>
                    {applyResult.written?.map((p) => (
                      <p key={p} className="font-mono text-green-400 break-all">✓ {p}</p>
                    ))}
                    {!!applyResult.backups?.length && (
                      <div className="pt-1 border-t border-green-500/20 mt-1">
                        <p className="text-muted-foreground">已备份原文件：</p>
                        {applyResult.backups.map((b) => <p key={b} className="font-mono text-muted-foreground break-all">{b}</p>)}
                      </div>
                    )}
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-white mb-1">现在在终端运行（Windows cmd 也适用）：</p>
                    <div className="relative group">
                      <pre className="bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-xs font-mono text-cyan-300 overflow-x-auto">{applyResult.launchCommand}</pre>
                      <Button size="sm" variant="ghost" onClick={() => copy(applyResult.launchCommand, "launch")}
                        className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 h-7 text-xs text-muted-foreground hover:text-white">
                        {copiedKey === "launch" ? <><CheckCircle2 className="w-3 h-3 mr-1 text-green-400" />已复制</> : <><Copy className="w-3 h-3 mr-1" />复制</>}
                      </Button>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1.5">--dangerously-bypass-approvals-and-sandbox 用于绕过 Windows 沙盒报错，让 Codex 直接运行。</p>
                  </div>
                </div>
              )}

              {/* 写入结果：容器降级 */}
              {applyResult && applyResult.notLocal && (
                <div className="space-y-3">
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3 text-xs text-amber-300 flex items-start gap-2">
                    <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                    <p>{applyResult.message}</p>
                  </div>
                  {applyResult.windowsScript && (
                    <div>
                      <p className="text-xs font-semibold text-white mb-1">方案 A：把下面脚本存为 <code className="text-cyan-300">配置Codex.bat</code> 在本机双击运行</p>
                      <div className="relative group">
                        <pre className="bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-xs font-mono text-gray-300 overflow-x-auto max-h-48">{applyResult.windowsScript}</pre>
                        <Button size="sm" variant="ghost" onClick={() => copy(applyResult.windowsScript!, "winscript")}
                          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 h-7 text-xs text-muted-foreground hover:text-white">
                          {copiedKey === "winscript" ? <><CheckCircle2 className="w-3 h-3 mr-1 text-green-400" />已复制</> : <><Copy className="w-3 h-3 mr-1" />复制</>}
                        </Button>
                      </div>
                    </div>
                  )}
                  {applyResult.configToml && (
                    <div>
                      <p className="text-xs font-semibold text-white mb-1">方案 B：手动把内容存为 <code className="text-cyan-300">~/.codex/config.toml</code></p>
                      <div className="relative group">
                        <pre className="bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-xs font-mono text-gray-300 overflow-x-auto max-h-48">{applyResult.configToml}</pre>
                        <Button size="sm" variant="ghost" onClick={() => copy(applyResult.configToml!, "cfgtoml")}
                          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 h-7 text-xs text-muted-foreground hover:text-white">
                          {copiedKey === "cfgtoml" ? <><CheckCircle2 className="w-3 h-3 mr-1 text-green-400" />已复制</> : <><Copy className="w-3 h-3 mr-1" />复制</>}
                        </Button>
                      </div>
                    </div>
                  )}
                  <div className="flex items-center gap-2 text-xs text-cyan-300 bg-cyan-500/5 border border-cyan-500/20 rounded-lg px-3 py-2">
                    <Terminal className="w-3.5 h-3.5 flex-shrink-0" /> 完成后运行：<span className="font-mono">{applyResult.launchCommand}</span>
                  </div>
                </div>
              )}

              {/* 写入结果：失败 */}
              {applyResult && !applyResult.ok && !applyResult.notLocal && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 text-xs text-red-300 flex items-start gap-2">
                  <XCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                  <p>{applyResult.message || "写入失败"}</p>
                </div>
              )}

              <div className="flex items-center gap-2 text-xs text-muted-foreground bg-background/40 border border-border/50 rounded-lg px-3 py-2">
                <span>使用模型 <span className="font-mono text-cyan-300">{result.resolvedModel}</span></span>
                <span className="text-muted-foreground/50">|</span>
                <span>API <span className="font-mono text-cyan-300">{result.openaiBaseUrl}</span></span>
              </div>
            </div>
          )}
        </div>
